...
reads_per_len.nmds = readRDS(file = "intermediate_RDS/KO_nmds_full.rds")
...
exclude_timpepoints = c(0,1,2,3,4,5)
#Time of day
ggplot(reads_per_len.nmds.metadata %>% filter(!TimePoint %in% exclude_timpepoints), aes(MDS1, MDS2, shape = Plot, color = as.numeric(timeOfDay.RNA))) +
geom_point(size = 3) +
my_gg_theme +
...
ggplot(reads_per_len.nmds.metadata %>% filter(!TimePoint %in% exclude_timpepoints), aes(MDS1, MDS2, shape = Plot, label = round(as.numeric(timeOfDay.RNA)), color = Plot)) +
geom_text(size = 3) +
my_gg_theme +
#labs(color = "Time of Day") +
#scale_color_gradient(low = "#0571b0", high = "#ca0020") +
...
pdf("KO_NMDS_axes_over_time.pdf", width = 24, height = 10)
grid.arrange(p1,p3,p5,p2,p4,p6, ncol = 3)
dev.off()


reads_per_len.wide.P1.nmds = readRDS(file = "intermediate_RDS/KO_nmds.P1.rds")
reads_per_len.wide.P2.nmds = readRDS(file = "intermediate_RDS/KO_nmds.P2.rds")
reads_per_len.wide.P3.nmds = readRDS(file = "intermediate_RDS/KO_nmds.P3.rds")
...
ggplot(reads_per_len.nmds.P1.P2.P3.metadata, aes(MDS2, MDS1, color = timeOfDay.RNA, label = round(as.numeric(timeOfDay.RNA)))) +
geom_text(size = 2) +
...
ggplot(reads_per_len.nmds.P1.P2.P3.metadata, aes(MDS2, MDS1, color = Temperature, label = round(as.numeric(timeOfDay.RNA)))) +
geom_text(size = 5) +
facet_wrap(~Plot, ncol = 3, scales = "free") +
my_gg_theme +
...
ggplot(reads_per_len.nmds.P1.P2.P3.metadata, aes(MDS2, MDS1, color = Moisture, label = round(as.numeric(timeOfDay.RNA)))) +
geom_text(size = 5) +
facet_wrap(~Plot, ncol = 3, scales = "free") +
my_gg_theme +
scale_color_gradient(low = "#2166ac", high = "#b2182b",trans = "sqrt") +
labs(color = "time of Day") +
theme(legend.title = element_text(size = 22))
...
pdf("KO_NMDS_axes_over_time_by_plot.pdf", width = 12, height = 6)
