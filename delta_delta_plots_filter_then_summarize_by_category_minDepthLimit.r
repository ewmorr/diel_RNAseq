...
catToSum = args[3]
outputFile = args[4]
minDepthAllowed = as.numeric(args[5])
...
minDepth = (colSums(read_count.filtered[,9:115]) %>% sort)[1]
print(c("minimum sequence depth: ", minDepth))

if(minDepth < as.numeric(minDepthAllowed)){
    print("minimum sequence depth is lower than minimum allowed.")
    print("Please enter a new rarefaction depth based on the depth per sample below.")
    print(colSums(read_count.filtered[,9:115]) %>% sort)
    minDepth = readLines(file("stdin"),1)
}
print(c("new minimum depth entered: ", minDepth))

#Rarefy then normalize by target len and summarize by KO for KO delta-delta analysis

read_count.rarefied = data.frame(read_count.filtered[,1:8], t(rrarefy(t(read_count.filtered[,9:115]), sample = minDepth)) )
