...
#1773
reads_per_len$Tax_name %>% unique %>% length
#493

reads_per_len$Edge_num = reads_per_len$Edge_num %>% as.factor

#look at totals by Tax_name
reads_per_len.Tax_name = reads_per_len %>% group_by(Tax_name) %>% summarize_if(is.numeric,sum,na.rm = TRUE)
reads_per_len.Tax_name.rowSum = data.frame(Tax_name = reads_per_len.Tax_name$Tax_name, sum = reads_per_len.Tax_name[2:length(colnames(reads_per_len.Tax_name))] %>% rowSums)
reads_per_len.Tax_name.rowSum[order(reads_per_len.Tax_name.rowSum$sum),]

top_50_genera = (reads_per_len.Tax_name.rowSum[order(reads_per_len.Tax_name.rowSum$sum, decreasing = T),])[1:50,]
top_50_genera$global_RA = top_50_genera$sum/sum(reads_per_len.Tax_name.rowSum$sum)

#Use top 50 taxa list to filter table for NLS fitting
#var names in mapped meta Plot, hours.cumulative.RNA

#look at totals by Edge_num
reads_per_len.Edge_num = reads_per_len %>% group_by(Edge_num, Tax_name) %>% summarize_if(is.numeric,sum,na.rm = TRUE)
reads_per_len.Edge_num.rowSum = data.frame(
    Edge_num = reads_per_len.Edge_num$Edge_num, 
    Tax_name = reads_per_len.Edge_num$Tax_name, 
    sum = reads_per_len.Edge_num[3:length(colnames(reads_per_len.Edge_num))] %>% rowSums
  )
reads_per_len.Edge_num.rowSum[order(reads_per_len.Edge_num.rowSum$sum, decreasing = T),]
#top "edges" are all mostly in the top taxa clades, e.g. Frigoribacterium, Rhizobium, Clavibacter, Erwinia, Massilia, Curtobacterium, Microbacteriaceae, etc

#Stick with taxonomic name summary for now
top_50_genera = (reads_per_len.Tax_name.rowSum[order(reads_per_len.Tax_name.rowSum$sum, decreasing = T),])[1:50,]
top_50_genera$global_RA = top_50_genera$sum/sum(reads_per_len.Tax_name.rowSum$sum)

write.table(top_50_genera, "top_50_genera.txt", quote = F, sep = "\t", row.names = F)
